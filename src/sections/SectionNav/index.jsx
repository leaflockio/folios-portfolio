...
/**
 * Right sidebar — section navigation with icons, paginated with arrows.
...
export function SectionNav({ profile }) {
  ...
  return (
    <div className="bg-[var(--color-text)]/5 fixed right-5 top-1/2 z-30 flex -translate-y-1/2 flex-col items-center gap-3 rounded-2xl px-2.5 py-3 backdrop-blur-sm">
      <button
          ...
          return (
            <button
              className={`group relative flex size-10 items-center justify-center rounded-xl transition-all ${
                isActive
                  ? 'bg-[var(--color-primary)]/15 text-[var(--color-primary)] opacity-100'
                  : 'hover:bg-[var(--color-primary)]/15 opacity-60 hover:text-[var(--color-primary)] hover:opacity-100'
              }`}
              key={section.id}
